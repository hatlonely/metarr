...
export function SettingsPage({ config, onSave, locale }: SettingsPageProps) {
  const text = t(locale);
  const [localConfig, setLocalConfig] = useState<AppConfig>(config);

  // Keep the form in sync when the persisted config loads (async) or changes
  // elsewhere; nothing else mutates it mid-edit, so this won't clobber input.
  useEffect(() => {
        ...
        >
          ...
          <Field label={text.namingPreset}>
            <Select
              ...
              onValueChange={(value) => setLocalConfig((prev) => ({ ...prev, namingPreset: value }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              ...
              <div className="mt-2 space-y-2.5 rounded-lg border bg-muted/30 p-3">
                {CUSTOM_TEMPLATE_FIELDS.map((field) => (
                  <div key={field} className="space-y-1">
                    <label className="text-xs text-muted-foreground">{templateLabels[field]}</label>
                    <Input
        ...
        >
          <Field label={text.historyMaxEntries} htmlFor="history-max-entries" hint={text.historyMaxEntriesHint}>
            <Input
              ...
              onChange={(e) =>
                setLocalConfig((prev) => ({ ...prev, historyMaxEntries: Number(e.target.value) || 0 }))
              }
            />
          </Field>
        ...
        <Section icon={SlidersHorizontal} title={text.settingsSectionGeneral}>
          <Field label={text.displayLanguage}>
            <Select
              ...
              onValueChange={(value) => setLocalConfig((prev) => ({ ...prev, displayLanguage: value }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
